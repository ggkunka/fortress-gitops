...
export class UserApi {
  ...
  async getUsers(page = 1, pageSize = 50, search?: string): Promise<PaginatedResponse<User>> {
    ...
    const response = await apiClient.get<ApiResponse<PaginatedResponse<User>>>(this.baseUrl, { params });
  ...
  async updateRole(id: string, role: Partial<UserRole>): Promise<UserRole> {
    const response = await apiClient.put<ApiResponse<UserRole>>(`${this.baseUrl}/roles/${id}`, role);
    if (!response.data.data) {
  ...
  async assignRole(userId: string, roleId: string): Promise<User> {
    const response = await apiClient.post<ApiResponse<User>>(`${this.baseUrl}/${userId}/roles/${roleId}`);
    if (!response.data.data) {
  ...
  async unassignRole(userId: string, roleId: string): Promise<User> {
    const response = await apiClient.delete<ApiResponse<User>>(`${this.baseUrl}/${userId}/roles/${roleId}`);
    if (!response.data.data) {
  ...
  async getUserPermissions(userId: string): Promise<Permission[]> {
    const response = await apiClient.get<ApiResponse<Permission[]>>(`${this.baseUrl}/${userId}/permissions`);
    return response.data.data || [];
  }
  ...
  async testAuthConfig(config: AuthConfig): Promise<{ success: boolean; message: string; details?: any }> {
    const response = await apiClient.post<ApiResponse<{ success: boolean; message: string; details?: any }>>(
      '/api/v1/auth/test-config',
      config
    );
  ...
  // LDAP/AD Integration
  async testLdapConnection(config: any): Promise<{ success: boolean; message: string; users?: any[] }> {
    const response = await apiClient.post<ApiResponse<{ success: boolean; message: string; users?: any[] }>>(
  ...
  async syncLdapUsers(): Promise<{ success: boolean; message: string; syncedUsers: number }> {
    const response = await apiClient.post<ApiResponse<{ success: boolean; message: string; syncedUsers: number }>>(
      '/api/v1/auth/sync-ldap-users'
    );
    return response.data.data || { success: false, message: 'Unknown error', syncedUsers: 0 };
  }
  ...
  async mapLdapGroup(ldapGroup: string, roleId: string): Promise<{ success: boolean; message: string }> {
  ...
  async getSsoProviders(): Promise<{ id: string; name: string; type: string; enabled: boolean }[]> {
    const response = await apiClient.get<ApiResponse<{ id: string; name: string; type: string; enabled: boolean }[]>>(
  ...
  async configureSsoProvider(provider: {
    ...
    const response = await apiClient.post<ApiResponse<{ success: boolean; message: string; providerId?: string }>>(
      '/api/v1/auth/configure-sso',
      provider
    );
    return response.data.data || { success: false, message: 'Unknown error' };
  }

  async testSsoProvider(providerId: string): Promise<{ success: boolean; message: string; loginUrl?: string }> {
    const response = await apiClient.post<ApiResponse<{ success: boolean; message: string; loginUrl?: string }>>(
  ...
  async resetPassword(userId: string): Promise<{ success: boolean; message: string; temporaryPassword?: string }> {
    const response = await apiClient.post<ApiResponse<{ success: boolean; message: string; temporaryPassword?: string }>>(
  ...
  // Bulk Operations
  async bulkCreateUsers(users: Omit<User, 'id' | 'createdAt' | 'updatedAt' | 'lastLogin'>[]): Promise<{
    success: boolean;
  ...
  async bulkAssignRoles(userIds: string[], roleIds: string[]): Promise<{
    success: boolean;
