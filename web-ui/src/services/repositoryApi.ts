...
export class RepositoryApi {
  ...
  async getImageRepository(id: string): Promise<ImageRepository> {
    const response = await apiClient.get<ApiResponse<ImageRepository>>(
  ...
  async createImageRepository(
    repository: Omit<ImageRepository, 'id' | 'lastSync' | 'imageCount' | 'vulnerabilityStats'>
  ): Promise<ImageRepository> {
    const response = await apiClient.post<ApiResponse<ImageRepository>>(
      `${this.baseUrl}/images`,
      repository
    );
    if (!response.data) {
  ...
  async updateImageRepository(
    id: string,
    repository: Partial<ImageRepository>
  ): Promise<ImageRepository> {
    const response = await apiClient.put<ApiResponse<ImageRepository>>(
      `${this.baseUrl}/images/${id}`,
      repository
    );
    if (!response.data) {
  ...
  async getChartRepository(id: string): Promise<ChartRepository> {
    const response = await apiClient.get<ApiResponse<ChartRepository>>(
      `${this.baseUrl}/charts/${id}`
    );
    if (!response.data) {
  ...
  async createChartRepository(
    repository: Omit<ChartRepository, 'id' | 'lastSync' | 'chartCount'>
  ): Promise<ChartRepository> {
    const response = await apiClient.post<ApiResponse<ChartRepository>>(
      `${this.baseUrl}/charts`,
      repository
    );
    if (!response.data) {
  ...
  async updateChartRepository(
    id: string,
    repository: Partial<ChartRepository>
  ): Promise<ChartRepository> {
    const response = await apiClient.put<ApiResponse<ChartRepository>>(
      `${this.baseUrl}/charts/${id}`,
      repository
    );
    if (!response.data) {
  ...
  async testImageRepositoryConnection(config: {
    ...
    >(`${this.baseUrl}/images/test-connection`, config);
  ...
  async testChartRepositoryConnection(config: {
    ...
    const response = await apiClient.post<
      ApiResponse<{ success: boolean; message: string; details?: any }>
    >(`${this.baseUrl}/charts/test-connection`, config);
    return response.data || { success: false, message: 'Unknown error' };
  }
  ...
  async deleteImage(repositoryId: string, imageName: string, tag?: string): Promise<void> {
    const url = tag 
      ? `${this.baseUrl}/images/${repositoryId}/images/${encodeURIComponent(imageName)}?tag=${tag}`
      : `${this.baseUrl}/images/${repositoryId}/images/${encodeURIComponent(imageName)}`;
  ...
  async getScanResult(scanId: string): Promise<ScanResult> {
    const response = await apiClient.get<ApiResponse<ScanResult>>(
  ...
  async getVulnerabilityStats(
    ...
    return (
      response.data || { total: 0, critical: 0, high: 0, medium: 0, low: 0, trend: 'stable' }
    );
  }
