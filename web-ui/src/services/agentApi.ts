...
export class AgentApi {
  ...
  async createAgent(agent: Omit<SecurityAgent, 'id' | 'deploymentStatus' | 'version'>): Promise<SecurityAgent> {
  ...
  async updateAgent(id: string, agent: Partial<SecurityAgent>): Promise<SecurityAgent> {
    const response = await apiClient.put<ApiResponse<SecurityAgent>>(`${this.baseUrl}/${id}`, agent);
    if (!response.data.data) {
  ...
  // Agent Deployment
  async deployAgent(agentId: string, clusterIds: string[]): Promise<{ deploymentId: string; status: string }> {
    const response = await apiClient.post<ApiResponse<{ deploymentId: string; status: string }>>(
      `${this.baseUrl}/${agentId}/deploy`,
      { clusterIds }
    );
    return response.data.data || { deploymentId: '', status: 'failed' };
  }

  async undeployAgent(agentId: string, clusterIds: string[]): Promise<{ success: boolean; results: any[] }> {
  ...
  async updateAgentDeployment(agentId: string, clusterIds: string[]): Promise<{ success: boolean; results: any[] }> {
    const response = await apiClient.post<ApiResponse<{ success: boolean; results: any[] }>>(
  ...
  async getAgentLogs(
    ...
    return response.data.data || { logs: [], podName: '' };
  }
  ...
  async getChartVersions(repository: string, chart: string): Promise<string[]> {
    const response = await apiClient.get<ApiResponse<string[]>>(
      `${this.baseUrl}/charts/versions`,
      { params: { repository, chart } }
    );
    return response.data.data || [];
  }

  async getChartValues(repository: string, chart: string, version: string): Promise<Record<string, any>> {
  ...
  async getAgentConfiguration(agentId: string): Promise<AgentConfiguration> {
    ...
    return response.data.data || {} as AgentConfiguration;
  }

  async updateAgentConfiguration(agentId: string, configuration: Partial<AgentConfiguration>): Promise<AgentConfiguration> {
  ...
  async createAgentFromTemplate(templateId: string, customization: Partial<SecurityAgent>): Promise<SecurityAgent> {
  ...
  // Agent Health Checks
  async performHealthCheck(agentId: string, clusterId: string): Promise<{
    healthy: boolean;
    ...
    return response.data.data || { healthy: false, checks: [] };
  }
  ...
  async getAgentMetrics(
    ...
    return response.data.data || { cpu: [], memory: [], scans: [], vulnerabilities: [] };
  }

  // Bulk Operations
  async bulkDeploy(agentIds: string[], clusterIds: string[]): Promise<{
    success: boolean;
  ...
  async bulkUndeploy(agentIds: string[], clusterIds: string[]): Promise<{
    success: boolean;
    results: { agentId: string; clusterId: string; success: boolean; message: string }[];
  }> {
    const response = await apiClient.post<ApiResponse<{
  ...
  async bulkUpdate(agentIds: string[], clusterIds: string[]): Promise<{
    success: boolean;
    results: { agentId: string; clusterId: string; success: boolean; message: string }[];
  }> {
    const response = await apiClient.post<ApiResponse<{
